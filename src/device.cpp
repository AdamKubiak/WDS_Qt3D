...
void Device::startCommunication()
{
    ...
    if(serialPort.open(QSerialPort::ReadWrite)) {
      ...
      {
      info = new QMessageBox(QMessageBox::Information,"Kalibracja","Trwa proces kalibracji trzymaj płytkę nieruchomo ... ",0,0,Qt::FramelessWindowHint| Qt::WindowStaysOnTopHint);
      info->setStandardButtons(0);
      info->setCursor(Qt::WaitCursor);
      info->show();
